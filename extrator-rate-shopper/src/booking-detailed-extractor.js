...
async function extract_prices_from_booking(url, start_date, end_date, results_filepath) {
  ...
  for (const date of dates) {
    ...
    try {
      ...
      // TODO: Implemnetar meios de identificar qual a capacidade de hóspedes em cada opção de quarto
      for (const price_table_row of prices_table_rows) {
        try {
          let price_element_text_value = await price_table_row.evaluate((el) => {
            return el.getAttribute("data-hotel-rounded-price")
          })

          console.log({ extracted_value: price_element_text_value })

          let parsed_monetary_value

          if (!price_element_text_value) {
            const price_column = await price_table_row.$(prices_column_selector)
            const price_element = await price_column.$(prices_element_selector)

            await price_table_row.dispose()
          ...
          if (parsed_monetary_value && /[0-9]+/.test(parsed_monetary_value)) {
            prices.push(Number(parsed_monetary_value))
          }
      ...
      console.log({ prices_before_sort: prices })

      prices.sort((a, b) => a - b)

      console.log({ prices_after_sort: prices })

      if (prices[0]) {
        const selected_price_string = String(prices[0])
