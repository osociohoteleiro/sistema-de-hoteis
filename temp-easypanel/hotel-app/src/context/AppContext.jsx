...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Buscar logotipo ativo após carregar configurações (apenas se tiver token)
    // Aguardar um tempo maior para garantir que o usuário fez login
    const checkForActiveLogo = () => {
      const token = localStorage.getItem('token');
      if (!token) {
        console.log('🔐 AppContext: Token não disponível, não carregando logotipo ativo');
        return;
      }
      ...
      const loadActiveLogo = async () => {
        try {
          ...
          const params = hotelToUse ? `?hotel_id=${hotelToUse}` : '';
          
          console.log('🖼️ AppContext: Buscando logotipo ativo para hotel:', hotelToUse);
          ...
          if (response.ok) {
            ...
            if (activeLogo) {
              console.log('🖼️ Logotipo ativo carregado no contexto:', activeLogo.logo_url);
              setConfig(prevConfig => ({ ...prevConfig, logo: activeLogo.logo_url }));
            } else {
              console.log('🖼️ Nenhum logotipo ativo encontrado');
            }
          } else {
            console.log('🖼️ Erro ao buscar logotipo ativo:', response.status);
          }
        } catch (error) {
          console.error('❌ Erro ao carregar logotipo ativo inicial:', error);
        }
      };
    ...
    setTimeout(checkForActiveLogo, 2000);
  }, []);
  ...
  const selectHotel = (hotelUuid) => {
    ...
    if (hotelUuid) {
      ...
      // Buscar logotipo ativo para o hotel selecionado
      fetchActiveLogo(hotelUuid);
    } else {
      localStorage.removeItem('selectedHotelUuid');
      
      // Buscar logotipo global quando nenhum hotel estiver selecionado
      fetchActiveLogo(null);
    }
