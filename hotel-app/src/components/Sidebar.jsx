...
const Sidebar = () => {
  ...
  console.log('🔍 Sidebar: appConfig:', appConfig);
  ...
  const handleItemClick = (item, path, e) => {
  ...
  // Função para verificar se o usuário pode acessar um subitem
  ...
  // Função para lidar com cliques em subitens sem permissão
  const handleSubitemClick = (subitem, path, e) => {
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sidebar-400 text-sm">
              {appConfig?.description || 'Painel Administrativo'}
            </p>
          </div>
        </div>
      </div>
            ...
            return (
              <div key={item.path}>
                {item.hasSubmenu ? (
                      ...
                      <div className="mt-2 ml-6 space-y-1">
                        {item.submenu?.map((subitem) => (
                          <Link
                            key={subitem.path}
                            to={subitem.path}
                            onClick={(e) => handleSubitemClick(subitem, subitem.path, e)}
                            className={`block px-4 py-2 rounded-lg text-sm transition-all duration-200 ${
                              location.pathname === subitem.path
                                ? 'bg-primary-500 text-white'
                                : 'text-sidebar-400 hover:bg-sidebar-800 hover:text-white'
                            } ${!canAccessSubitem(subitem) ? 'opacity-75' : ''} flex items-center justify-between`}
                          >
                            <span>{subitem.name}</span>
                            {!canAccessSubitem(subitem) && (
                  ...
                  <Link
                    to={item.path}
                    onClick={(e) => handleItemClick(item, item.path, e)}
                  ...
                  >
                    ...
                    <span className="font-medium">{item.name}</span>
                    {!canAccessItem(item) && (
                      <svg className="w-4 h-4 text-yellow-400 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                      </svg>
                    )}
                  </Link>
