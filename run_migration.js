...
async function runMigration() {
  const client = new Client({
    user: 'postgres',
    host: 'localhost',
    database: 'osh_db', // ou osh-ia, conforme visto no health check
  ...
  try {
    ...
    const migrationPath = path.join(__dirname, 'api', 'migrations', '021_add_platform_to_rate_shopper_properties.sql');
