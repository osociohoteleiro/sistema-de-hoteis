...
const AddWorkspaceModal = ({ isOpen, onClose, hotel, onWorkspaceAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (onWorkspaceAdded) {
          onWorkspaceAdded(response.data.data);
        }

        // Fechar modal
        onClose();
      } else {
        setError(response.data.message || 'Erro ao criar workspace');
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        ...
        <div className="bg-gradient-sapphire px-6 py-4">
          ...
          <p className="text-sapphire-100 text-sm mt-1">
            Adicionar nova workspace para {hotel?.name}
          </p>
        </div>
